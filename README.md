...
**v1.5.0 更新**：架构全面升级。从线性「检查清单」模式重构为**六层嵌套架构**
体裁识别 → 模式识别 → 作者声音识别 → 判断决策 → 改写执行 → 质量控制。新增路由分发（根据 AI 味浓度自动选择策略）和迭代精炼（改写 → 验证 → 再改写，直到达标）。19 维图谱从 12 维扩展，判断逻辑从启发式三问升级为决策树。核心skill也进行了扩展到5个skill，可以覆盖更多的场景。
