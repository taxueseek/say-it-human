...
读者判断一段文字是不是 AI 写的，不是因为用词不够华丽——是因为感觉不到背后有一个人在思考和创作。AI 味不是「写得不好」，是「太可预测了」。读到上一句就猜到下一句，读到开头就知道结尾。这种可预测性才是问题的根源。

**Say It Human** 是一套 AI 味检测与消除工具，内置 19 维 AI 味来源图谱，覆盖 2026 年最新特征。可在 Claude Code、Codex、Cursor 等任意支持 skill 的 Agent 上使用。

**核心理念**：去 AI 味不是让文字「更好看」——是让读者感觉到「背后有一个真人在做判断」。

最新版本：**v1.5.0**

**v1.5.0 更新**：架构全面升级。从线性「检查清单」模式重构为**六层嵌套架构**——体裁识别 → 模式识别 → 作者声音识别 → 判断决策 → 改写执行 → 质量控制。新增路由分发（根据 AI 味浓度自动选择策略）和迭代精炼（改写 → 验证 → 再改写，直到达标）。19 维图谱从 12 维扩展，判断逻辑从启发式三问升级为决策树。
